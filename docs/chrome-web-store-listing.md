...
- 修复 Chrome 加载扩展时报 web_accessible_resources Invalid match pattern 的问题。
- Web Accessible Resources 仅使用 Chrome 接受的来源级 /* 匹配模式。
- HuggingChat 等路径限定站点改为运行时页面路径校验，避免在普通页面显示增强入口。
